  ...
  Widget addContactWidget(BuildContext context){
    return Column(
        children: [
          ...
          const SizedBox(height: 20),
          const Text('Silakan masukkan nama dan nomor telepon yang ingin anda masukkan kedalam daftar kontak'),
          const SizedBox(height: 20),
          const Divider(
          color: Colors.grey,
          height: 5, 
          ...
          Form(key: formKey, 
            child: Column(
              children: [
                ...
                const SizedBox(height: 20),
                SizedBox(width: 550, child: TextFormField(
                  ...
                  decoration:  const InputDecoration(
                    ...
                    label: Text('Nomor', style: TextStyle(color: Colors.black),),
                    hintText: '+62 ...'
                  ),
                ...
                (fileBuka == null) ? Container() : SizedBox(
                  width: 100,
                  height: 100,
              ...
              if(form!.validate()){
                setState(() {
                    ...
                    contact.add(AddContactList(nama: nama, number: number, date: date, warna: warna, file: file));
                    logDev.log(contact.toString(),name:"MyLog");
                  });   
              }
              ...
              child: const Text('Save Edit'))
                ...
                const Padding(padding: EdgeInsets.only(top: 50)),
                  Title(color: Colors.black, child: const Text('List Contacts',style: TextStyle(fontSize: 24))),
                  const Padding(padding: EdgeInsets.only(top: 50)),
                   contact.isEmpty ? const Text('Belum ada Kontak yang ditambahkan', style: TextStyle(fontSize: 20),): 
               SizedBox(
                height: 200,
 ...
 Widget getList(int index) {
  return ListTile(
    leading: CircleAvatar(
      backgroundColor: const Color.fromARGB(255, 126, 48, 139).withOpacity(0.2),
      child: Text(contact[index].nama[0].toUpperCase(), 
      style: const TextStyle(color: Color.fromARGB(255, 96, 4, 112)),),),
    title:Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        Text(contact[index].nama, style: const TextStyle(fontWeight: FontWeight.bold)),
        Text(contact[index].number.toString()),
        Text('Date Added : ${contact[index].date?.day}-${contact[index].date?.month}-${contact[index].date?.year}'),
        Row(
          children: [
            const Text('Color : '),
